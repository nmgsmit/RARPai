...
def main():
    ...
    ap.add_argument("--root", default=str(A), help="dir of packed videos (default arch_tip_all; e.g. arch_tip_pure)")
    args = ap.parse_args()
    ...
    root = Path(args.root)
    for d in sorted(p for p in root.iterdir() if (p / "frames.npy").exists()):
        store = FrameStore(d.name, root=root)
        N = len(store)
