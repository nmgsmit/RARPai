...
def main():
    ...
    rows = []
    for video in sorted({r["video"] for r in keep}):
        ...
        off = {n: (np.zeros(2) if n == REFERENCE else offset_for(video, n, {(m, video): ann[m] for m in names}))
               for n in names}
        want = {r["frame"]: r for r in keep if r["video"] == video}
        short = patient(video)[:8]
        cap, box, i = cv2.VideoCapture(str(DATA / video)), None, -1
        ...
        cap.release()
        print(f"{video}: {sum(r['video'] == video for r in rows)} frames")
