...
def predict(model, img: Image.Image, size: int, device) -> np.ndarray:
    """Class-id map at the image's ORIGINAL size."""
    x = np.array(img.convert("RGB").resize((size, size), Image.BICUBIC)) / 255.0
...
def main():
    ...
    ap.add_argument("--width", type=int, default=1100, help="px per tile in the sheet")
    args = ap.parse_args()

    legend = NEW if args.scheme == "new" else OLD
    ...
    print(f"[model] {args.checkpoint} num_classes={nc} device={device}")

    rows = []
    for p in picks:
        img = Image.open(p).convert("RGB")
        m = predict(model, img, args.img_size, device)
        raw = np.array(img)
        ov = raw.copy()
        ...
        tile = tile.resize((args.width, round(args.width * tile.height / tile.width)))
    ...
    strip = Image.new("RGB", (args.width, 26), (16, 16, 16))
